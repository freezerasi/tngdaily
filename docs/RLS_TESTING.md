...
| Direct `authenticated` INSERT into `user_roles` by roleless user | ❌ RLS denied (`user_roles_manage`) |
| Direct `authenticated` INSERT into `user_roles` by non-owner | ❌ RLS denied (`user_roles_manage`) |
| Direct client attempt to set `tng.bootstrap_owner = 'on'` and INSERT | ❌ RLS denied (`user_roles_manage`) |
| Primary bootstrap RPC called by first active authenticated user | ✅ Returns `{"ok": true}` |
| Primary bootstrap RPC called with `auth.uid() IS NULL` / `anon` | ❌ Denied (no execute grant / exception) |
| Primary bootstrap RPC called by `service_role` | ❌ Permission denied (execute revoked) |
| Primary bootstrap RPC called when active owner already exists | ❌ Exception (`restrict_violation`) |
| Primary bootstrap RPC cannot bootstrap another user | ✅ Structurally enforced (takes no user argument) |
| Primary bootstrap RPC cannot assign non-owner role | ✅ Structurally enforced (locks role to 'owner') |
| Primary bootstrap RPC produces owner role row with no expiry | ✅ `expires_at IS NULL` |
| Primary bootstrap RPC creates immutable audit event | ✅ `actor_id = auth.uid()`, `before_data = null`, `after_data = null`, `metadata = {"bootstrap_mode": "primary_authenticated"}` |
| Two concurrent primary bootstrap RPC calls | ✅ Exactly 1 succeeds, serialized via advisory lock |
| Emergency break-glass remains the only actor-null owner recovery path | ✅ Dedicated DB-owner function |

### Break-Glass

| Test | Expected |
| --- | --- |
| Call from `authenticated` role | ❌ Permission denied |
| Call from `service_role` | ❌ Permission denied |
| Call from database owner, zero owners | ✅ |
| Call from database owner, one owner exists | ❌ Exception |
| Call with inactive target | ❌ Exception |
...
This applies all migrations (0001–0013) and seeds.
...
- [RLS_POLICY_MIGRATION_REVIEW.md](./RLS_POLICY_MIGRATION_REVIEW.md) — Policy-by-policy intent
- [RBAC.md](./RBAC.md) — Full RBAC architecture
- [OWNER_BOOTSTRAP.md](./OWNER_BOOTSTRAP.md) — Bootstrap procedures
...
4. RPC tng_assign_role called by Anon (Must fail: execute denied).
5. RPC tng_assign_role called by Authenticated user without user.manage_roles (Must fail: missing permission).
6. RPC tng_assign_role called by Managing Editor (Rank 70) attempting to assign Editor (Rank 50) (Success).
7. RPC tng_assign_role called by Managing Editor attempting to assign Managing Editor (Must fail: target authority >= actor authority).
8. RPC tng_assign_role called by Editor (if granted user.manage_roles) attempting to assign Commercial Manager (both rank 50) (Must fail: target authority >= actor authority).
9. RPC tng_assign_role attempting to assign owner role (Must fail: owner role blocked via normal RPC).
10. RPC tng_assign_role attempting self-assignment (Must fail: self-assignment forbidden).
11. RPC tng_assign_role with malformed role key (Must fail: invalid format).
12. RPC tng_assign_role targeting inactive account (Must fail: target account not active).
13. RPC tng_assign_role creates correct audit event via tng_write_audit_log with actor ID matching JWT.
14. RPC tng_assign_role fails and rolls back if audit writer fails.
15. RPC tng_update_role_expiry called by non-Owner (Must fail: missing permission).
16. RPC tng_update_role_expiry extending expiry to future date (Success, audit change = extend_expiry).
17. RPC tng_update_role_expiry shortening expiry to earlier future date (Success, audit change = shorten_expiry).
18. RPC tng_update_role_expiry setting expiry where previously permanent (Success, audit change = set_expiry).
19. RPC tng_update_role_expiry with past date (Must fail: expiry in past).
20. RPC tng_update_role_expiry with null date (Must fail: null not allowed).
21. RPC tng_update_role_expiry with exact same date (Must fail: no-op).
22. RPC tng_update_role_expiry creates correct audit event with correct change type.
23. RPC tng_revoke_role called on active target account (Success).
24. RPC tng_revoke_role called on inactive/suspended target account (Success).
25. RPC tng_revoke_role attempting to revoke owner (Must fail: owner role blocked via normal RPC).
26. RPC tng_revoke_role attempting self-revocation (Must fail: self-revocation forbidden).
...
37. Concurrency: Two overlapping tng_update_role_expiry calls on the same target block correctly via FOR UPDATE and do not produce conflicting audit change types.
