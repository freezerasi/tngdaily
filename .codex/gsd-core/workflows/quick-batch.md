...
Validate `{{GSD_ARGS}}` through the CLI's own grammar — never re-derive it inline (single source of truth: `parseQuickBatchArgs`, `src/quick-batch-dispatch.cts`). `{{GSD_ARGS}}` is raw, attacker-influenced task text — pass it as ONE quoted argument via `--text` so the shell never word-splits or glob-expands it; `quick-batch parse-args` does the whitespace split itself, in Node, after the shell is done:

```bash
QB_PARSE_JSON=$(gsd_run quick-batch parse-args --raw --text "{{GSD_ARGS}}")
QB_PARSE_RC=$?
...
Extract the raw task-list text / `--file <path>` from `{{GSD_ARGS}}` (everything that is not `--jobs <v>`, `--validate`, `--research`, `--resume <id>`, or `--file <path>`'s own flag pair).
