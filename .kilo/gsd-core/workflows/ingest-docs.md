...
If user passed `--mode new` but `.planning/` already exists: display warning and require explicit confirm via `question` (approve-revise-abort from `gsd-core/references/gate-prompts.md`) before overwriting.
