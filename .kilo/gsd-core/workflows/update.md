...
for cand in \
  ...
  "C:/Users/Athira/Desktop/tngdaily/.kilo/gsd-core/bin/gsd-tools.cjs"; do
...
The `GSD_DIR` value emitted by `get_installed_version` (line 4) resolves to the runtime-specific config dir (`C:/Users/Athira/Desktop/tngdaily/.kilo/`, `~/.gemini/`, `~/.codex/`, etc.), so the script invocation works for every runtime — not just the agent. An unresolved target exits in `get_installed_version` before this step.
...
global: `C:/Users/Athira/Desktop/tngdaily/.kilo/`, `~/.config/opencode/`, `~/.opencode/`, `~/.gemini/`, `~/.config/kilo/`, or `~/.codex/`
